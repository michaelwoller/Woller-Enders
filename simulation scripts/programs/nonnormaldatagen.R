...
if(runfromshell){
  ...
  RUNONCLUSTER <- as.numeric(RUNVARS[8])
  WRKDIR <- RUNVARS[9]
} else {
...
muY <- 0 
varY <- 1 #issues pre setting this other than generating it
...
Y.star <- rep(beta0,N) + Xs %*% beta + Es

### generating the yeo-johnson, n =50, 

Y <- VGAM::yeo.johnson(Y.star, lambda = lambda, inverse = TRUE)
