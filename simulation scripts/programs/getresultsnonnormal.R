...
if(runfromshell){
  ...
  lambda <- as.numeric(RUNVARS[7])
  alpha <- as.numeric(RUNVARS[8:10])
  RUNONCLUSTER <- as.numeric(RUNVARS[11])
} else {
...
MLRCresults <- cbind(Fcoeff,FcoeffSE)
...
MLRWresults <- cbind(FWald, FWdf, FWp, sig.at.10, sig.at.05, sig.at.01)
...
MLCresults <- cbind(FNRcoeff,FNRcoeffSE)
...
MLWresults <- cbind(FNRWald, FNRWdf, FNRWp, sig.at.10, sig.at.05, sig.at.01)
...
# This extracts relevant output from the Blimp scripts
...
Bresults <- scan("BlimpBayesYJestimates.dat") 
...
PSRTresults <- cbind(REP, N, RSQ, numX, psr[20,])
...
waldresults.out <- cbind(REP, N, RSQ, numX, lambda, MLWresults, MLRWresults, BWresults, converge, BTWresults, convergeT)
coeffresults.out <- cbind(REP, N, RSQ, numX, lambda, MLCresults, MLRCresults, BCresults, BTCresults, coeffIndex)


Coeff.path <- paste0(WRKDIR, "/finalized/CoeffNonNormal")
Wald.path <- paste0(WRKDIR, "/finalized/WaldNonNormal")
psr.path <- paste0(WRKDIR, "/finalized/psrNonNormal")
psrt.path <- paste0(WRKDIR, "/finalized/psrYJNonNormal")

# Saving Coefficient output
setwd(Coeff.path)
Coeff.filename <- paste0("Coeffsimresults", ".N", N, ".NUMX", numX, ".RSQ", RSQ, ".LAMB", lambda, ".REP", REP, ".dat")
gdata::write.fwf(coeffresults.out, Coeff.filename, rownames = F, colnames = F, width = rep(15, ncol(coeffresults.out)))

# Saving Wald output
setwd(Wald.path)
Wald.filename <- paste0("Waldsimresults", ".N", N, ".NUMX", numX, ".RSQ", RSQ, ".LAMB", lambda, ".REP", REP, ".dat")
gdata::write.fwf(waldresults.out, Wald.filename, rownames = F, colnames = F, width = rep(15, ncol(waldresults.out)))

# Saving PSR output
setwd(psr.path)
psr.filename <- paste0("PSR", ".N", N, ".NUMX", numX, ".RSQ", RSQ, ".LAMB", lambda, ".REP", REP, ".dat")
